...
(function() {
    ...
    function parse_kube_config(data, context_name) {
        ...
        if (cluster) {
            if (cluster.server) {
                ...
                if (parser.protocol == 'https:') {
                    ...
                    options.tls.authority = parse_cert_option(cluster, "certificate-authority");
        ...
        if (user) {
            ...
            if (user.username)
                options.headers["Authorization"] = "Basic " + basic_token(user.username, user.password || "");
            if (options.tls) {
                options.tls.certificate = parse_cert_option(user, "client-certificate");
                options.tls.key = parse_cert_option(user, "client-key");
            }
        }
    ...
    .factory("cockpitKubeDiscover", [
        ...
        function($q, CockpitKubeRequest, cockpitKubectlConfig) {
            ...
            return function cockpitKubeDiscover(force) {
                ...
                function step(options, kube_config) {
                    if (!options)
                ...
                var kubectl = cockpitKubectlConfig()
                    .then(function(data) {
                        var options = parse_kube_config(data);
